...
@dp.message_handler(text='Userni tekshirish 🔎',chat_id = ADMINS)
async def check_user(msg: types.Message):
    await msg.answer('Tekshirmoqchi bo\'lgan foydalanuvchingizni IDsini kiriting:')
    await InfoUser.user_id.set()
@dp.message_handler(state=InfoUser.user_id)
async def info(msg: types.Message,state: FSMContext): 
    user_id = msg.text 
    try:
        await msg.answer(f'Userning ma\'lumotlari:\n'
                     f'👤Ismi: {get_user(user_id=user_id)}\n'
                     f'🔑ID: {user_id}\n'
                     f'💳Hisob: {get_phone(user_id)}\n'
                     f'💰Balans: {get_balance(user_id)}\n')
        await state.finish()
